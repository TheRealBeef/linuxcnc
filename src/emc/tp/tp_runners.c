...
int rtapi_app_main(void)
{
    ...
    char* emsg;
    tpmod_id = hal_init("tpmod_runners"); // dlopen(".../tpmod.so")
    if (tpmod_id < 0) {emsg="hal_init()"; goto error;}

    int r=0;

    ruckig_x_pos = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("tpmod_scurve_skynet.ruckig_x_pos",HAL_OUT,&(ruckig_x_pos->Pin),tpmod_id);

    ruckig_x_vel = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("tpmod_scurve_skynet.ruckig_x_vel",HAL_OUT,&(ruckig_x_vel->Pin),tpmod_id);

    ruckig_x_acc = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("tpmod_scurve_skynet.ruckig_x_acc",HAL_OUT,&(ruckig_x_acc->Pin),tpmod_id);

    ruckig_y_pos = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("tpmod_scurve_skynet.ruckig_y_pos",HAL_OUT,&(ruckig_y_pos->Pin),tpmod_id);

    ruckig_y_vel = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("tpmod_scurve_skynet.ruckig_y_vel",HAL_OUT,&(ruckig_y_vel->Pin),tpmod_id);

    ruckig_y_acc = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("tpmod_scurve_skynet.ruckig_y_acc",HAL_OUT,&(ruckig_y_acc->Pin),tpmod_id);

    ruckig_z_pos = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("tpmod_scurve_skynet.ruckig_z_pos",HAL_OUT,&(ruckig_z_pos->Pin),tpmod_id);

    ruckig_z_vel = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("tpmod_scurve_skynet.ruckig_z_vel",HAL_OUT,&(ruckig_z_vel->Pin),tpmod_id);

    ruckig_z_acc = (float_data_t*)hal_malloc(sizeof(float_data_t));
    r+=hal_pin_float_new("tpmod_scurve_skynet.ruckig_z_acc",HAL_OUT,&(ruckig_z_acc->Pin),tpmod_id);
...
inline void update_ruckig_followers(TP_STRUCT * const tp){
        ...
        rx.tarpos=tp->currentPos.tran.x;
        ...
        ry.tarpos=tp->currentPos.tran.y;
        ...
        rz.tarpos=tp->currentPos.tran.z;
...
STATIC tp_err_t tpCreateLineArcBlend(TP_STRUCT * const tp, TC_STRUCT * const prev_tc, TC_STRUCT * const tc, TC_STRUCT * const blend_tc)
{
    ...
    tpInitBlendArcFromPrev(tp, prev_tc, blend_tc, param.v_req,
                           param.v_plan, param.a_max, tc->max_jerk);
...
int tpGetPos(TP_STRUCT const * const tp, EmcPose * const pos)
{
    ...
    if (0 == tp) {
        ...
        *pos = tp->currentPos;
        ...
        //! Update pos from the scurve runners.
        pos->tran.x=rx.curpos;
        pos->tran.y=ry.curpos;
        pos->tran.z=rz.curpos;
